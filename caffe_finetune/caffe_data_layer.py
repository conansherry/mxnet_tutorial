...
class CaffeDataLayer(caffe.Layer):
    ...
    def setup(self, bottom, top):
        ...
        if len(top) != 2:
            raise Exception("Need to define three tops: data and label, edge_label")
        # data layers have no bottoms
        if len(bottom) != 0:
            raise Exception("Do not define a bottom.")

        self.data_iter = FileIter(r'E:/clean_data')

    def reshape(self, bottom, top):
        dataset = self.data_iter.next()
        self.data = dataset['data']
        self.label = dataset['l2_label']
